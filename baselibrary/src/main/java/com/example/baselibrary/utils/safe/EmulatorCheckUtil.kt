...
import com.newsclapper.base.utils.safe.EmulatorCheckCallback
...
class EmulatorCheckUtil private constructor() {
     ...
     * 如果是多进程，推荐使用进程名 SecurityCheckUtil#getCurrentProcessName()
     * @return
     */
    fun checkByCreateLocalServerSocket(uniqueMsg: String?): Boolean {
        return if (localServerSocket != null) false else try {
            localServerSocket = LocalServerSocket(uniqueMsg)
            false
        } catch (e: IOException) {
            true
        }
