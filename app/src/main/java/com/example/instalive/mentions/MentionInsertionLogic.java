...
class MentionInsertionLogic {
    ...
    MentionInsertionLogic(final EditText editText) {
        ...
        this.textHighlightColor = R.color.default_yellow;
    }
     ...
     */
     void insertMention(final Mentionable mention) {
        ...
        if (start != -1) {
            final int newCursorPosition = start + mention.getMentionName().length() + 2;
            editText.getText().delete(start+1, cursorPosition);
            editText.getText().insert(start+1, mention.getMentionName() + " ");
            ...
            if (newCursorPosition < editText.length()) {
