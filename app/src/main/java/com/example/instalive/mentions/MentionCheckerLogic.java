...
class MentionCheckerLogic {
    ...
    String doMentionCheck() {
        ...
        if (StringUtils.contains(editText.getText(), "@")) {
            ...
            final String providedSearchText = StringUtils.substringAfterLastNullable(allTextBeforeCursor, "@");

            if (providedSearchText == null) return "@";
            ...
            if (searchIsWithinMaxChars(providedSearchText, maxCharacters)) {
                final String text = allTextBeforeCursor.substring(allTextBeforeCursor.length() - providedSearchText.length() -1);
                final int atSymbolPosition = StringUtils.lastIndexOf(text, "@");
    ...
    private boolean atBeforeAtSymbol(final String currentTextBeforeCursor, final int atSymbolPosition) {
        if (atSymbolPosition > 0) {
            final char charBeforeAtSymbol = currentTextBeforeCursor.charAt(atSymbolPosition - 1);
            if ((charBeforeAtSymbol == '@')) {
                return true;
            }
        }
        return false;
