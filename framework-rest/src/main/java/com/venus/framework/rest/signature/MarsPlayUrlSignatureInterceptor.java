 ...
 */
public class MarsPlayUrlSignatureInterceptor implements Interceptor, HttpConstants {

    @NonNull
    protected final UrlSignatureConfig config;

    @Inject
    public MarsPlayUrlSignatureInterceptor(@NonNull UrlSignatureConfig config) {
        this.config = config;
    }
    ...
    protected String signUrl(@NonNull String method,
        ...
        try {
            return UrlSignature.getInstance().signMarsPlayUrl(config.getContext(), method, url, params, config.getTracker());
        } catch (Exception e) {
            return null;
        }
    }
