 ...
 */
public class VenusPlayUrlSignatureInterceptor implements Interceptor, HttpConstants {

    @NonNull
    protected final UrlSignatureConfig config;

    @Inject
    public VenusPlayUrlSignatureInterceptor(@NonNull UrlSignatureConfig config) {
        this.config = config;
    }
    ...
    protected String signUrl(@NonNull String method,
        ...
        try {
            return UrlSignature.getInstance().signVenusPlayUrl(config.getContext(), method, url, params, config.getTracker());
        } catch (Exception e) {
            return null;
        }
    }
